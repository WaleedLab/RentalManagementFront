...
export class InsuranceCompanyListComponent implements OnInit {
  ...
  private api = inject(InsuranceCompanyService);
  private toast = inject(ToastService);
  private translate = inject(TranslateService);
  private confirm = inject(ConfirmService);

  rows = signal<InsuranceCompany[]>([]);
    ...
    this.api
      ...
      .subscribe({
        ...
        error: err => {
          ...
          this.toast.error(err?.message ?? this.translate.instant('maintenance.insurance.loadFailed'));
        },
  ...
  deleteRow(row: InsuranceCompany): void {
    ...
    this.confirm
      .confirm(
        this.translate.instant('maintenance.insurance.deleteTitle'),
        `${this.translate.instant('maintenance.insurance.deleteConfirm')} ${label}`,
      )
      .subscribe(confirmed => {
        if (!confirmed) return;
        ...
        this.api.softDelete(row.id, fleetId).subscribe({
          next: () => {
            this.toast.success(this.translate.instant('maintenance.insurance.deleteSuccess'));
            this.load();
          },
          error: err =>
            this.toast.error(err?.message ?? this.translate.instant('maintenance.insurance.deleteFailed')),
          complete: () =>
